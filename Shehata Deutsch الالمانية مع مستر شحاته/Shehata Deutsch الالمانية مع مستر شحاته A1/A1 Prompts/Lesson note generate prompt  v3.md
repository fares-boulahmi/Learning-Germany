Got it — I'll remove the permanent base list from the prompt body and keep only the per-session input field at the bottom. Here's the clean version:
...
You receive a raw copy-paste from a German grammar PDF, and a list of words the student already knows. It may contain **multiple lessons** (anywhere from 1 to 7+). It may be messy: Arabic mixed in, broken formatting, repeated headers, OCR artifacts, duplicate exercises.
...
3. Sentence vocabulary must stay strictly within the student's known word list — no extra new words sneaking in through examples.
...
- Do not introduce any vocabulary beyond this
- The Part 2 cap of 3 new words per lesson is the absolute ceiling — do not exceed it across exercises either
...
- Do not let Part 2 example sentences introduce vocabulary outside the known word list

---

## Input

**My known words:** [PASTE YOUR WORD LIST HERE]
